    ...
    if request.method == 'POST':
        ...
        if len(doc.ents) > 0:
            ...
            if choice == 'organization':
                results = ORG_named_entity['value']
                num_of_results = len(results)
            elif choice == 'person':
                results = PERSON_named_entity['value']
                num_of_results = len(results)
            elif choice == 'geopolitical':
                results = GPE_named_entity['value']
                num_of_results = len(results)
            elif choice == 'money':
                results = MONEY_named_entity['value']
                num_of_results = len(results)
...
def get_entities_normalized():
    ...
    # transform pasangan label menjadi dataframe 
    df = pd.DataFrame(d, columns=['category', 'value'])
    df = df.groupby('category').agg({'value': lambda x: ','.join(x)})
    df = df['value'].apply(lambda x: x.split(','))
